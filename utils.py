...
optimizer = tf.keras.optimizers.legacy.Adam(learning_rate=1e-4) # updated to legacy to better run on M3 mac
...
def plot_grid(images,N=10,C=10,figsize=(24., 28.),name='posterior'):
    ...
    is_grayscale = images.ndim == 4 and images.shape[-1] == 1 #do grayscale check
    for ax, im in zip(grid, images):
        # Iterating over the grid returns the Axes.
        if is_grayscale:
            ax.imshow(im.squeeze(-1), cmap='gray', vmin=0.0, vmax=1.0) #force grayscale
        else:
            ax.imshow(im, vmin=0.0, vmax=1.0)
        ax.set_xticks([])
        ax.set_yticks([])
    prefix = 'xhat_color_' if not is_grayscale else 'xhat_bw_'
    plt.subplots_adjust(wspace=0, hspace=0)
    plt.savefig(f'./{prefix}{name}.pdf') #better naming convention
    plt.close()
